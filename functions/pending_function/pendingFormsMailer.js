...
function buildConsolidatedPendingFormsHtml({ periodLabel, asOfLabel, siteDigests }) {
  const asOf = asOfLabel || periodLabel;
  const logoBlock = vayonaLogoHtml();
  const siteSections = (siteDigests || [])
    .map((site) => {
      const siteLabel = siteDisplayName(site.siteName);
      const rows = (site.forms || [])
        .map((form, idx) => {
          const border = idx === site.forms.length - 1 ? '0' : '1px solid #e4edef';
          return `<tr>
          <td align="center" valign="middle" style="padding:14px 8px;border-bottom:${border};">${rowIndexBadge(idx + 1)}</td>
          <td valign="middle" style="padding:14px 12px;border-bottom:${border};font-family:Arial,Helvetica,sans-serif;font-size:14px;font-weight:700;color:#111827;">${escapeHtml(form.formName)}</td>
          <td valign="middle" style="padding:14px 12px;border-bottom:${border};font-family:Arial,Helvetica,sans-serif;font-size:13px;line-height:1.45;color:#334155;">${escapeHtml(form.act || '—')}</td>
        ...
        .join('');
      return `<tr>
        <td style="padding:18px 32px 8px;font-family:Arial,Helvetica,sans-serif;font-size:18px;font-weight:700;color:#111827;">${escapeHtml(siteLabel)}</td>
      </tr>
      <tr>
        <td style="padding:8px 32px 18px;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border:1px solid #dbe5e8;">
...
<head>
  <meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Pending forms notification</title>
</head>
<body style="margin:0;padding:0;background-color:#f4f7f8;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#f4f7f8" style="background-color:#f4f7f8;">
    <tr>
      <td align="center" style="padding:0;">
        <table role="presentation" width="920" cellpadding="0" cellspacing="0" border="0" bgcolor="#ffffff" style="width:920px;max-width:920px;background-color:#ffffff;">
            ...
            <td bgcolor="#005c61" style="padding:28px 32px;background-color:#005c61;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
                <tr>
                  ...
                  <td valign="middle" style="padding-left:22px;font-family:Arial,Helvetica,sans-serif;">
                    <span style="display:block;font-size:28px;line-height:34px;font-weight:700;color:#ffffff;">Pending forms notification</span>
                    <span style="display:block;font-size:16px;line-height:24px;color:#d7eeee;padding-top:8px;">${escapeHtml(periodLabel)}</span>
                  </td>
                </tr>
              </table>
            </td>
          ...
          <tr>
            <td style="padding:28px 32px 12px;font-family:Arial,Helvetica,sans-serif;color:#111827;">
              <p style="margin:0;font-size:15px;line-height:1.65;color:#1f2937;">
                The following report forms are yet to be completed as of <strong>${escapeHtml(asOf)}</strong>. Please review and complete them in the <strong>HR Compliance Management Tool</strong>.
              </p>
            </td>
          </tr>
          ${siteSections || '<tr><td style="padding:28px 32px;font-family:Arial,Helvetica,sans-serif;color:#6b7280;">No pending forms found.</td></tr>'}
          <tr>
            <td style="padding:16px 32px 28px;font-family:Arial,Helvetica,sans-serif;font-size:13px;color:#6b7280;">This is an automated notification email.</td>
          </tr>
        </table>
      </td>
    </tr>
...
/**
 * Compile pending forms across all sites and email only the configured recipient(s).
 * Does not send mail to Site In-Charge or any Site Management email.
 */
async function sendPendingFormsToRecipients(catalyst, options = {}) {
  const now = nowInKolkata();
  const periodLabel = currentPeriodLabel(now);
  const asOfLabel = formatAsOfLabel(now);
  const dryRun = Boolean(options.dryRun);
  const recipientEmails = (options.recipientEmails || [])
    .map((email) => normalizeEmail(email))
  ...
  for (const site of siteRows) {
    ...
    siteDigests.push({
      siteName,
      inchargeName: pickField(site, 'InchargeName', 'inchargeName'),
      forms: forms.map((f) => ({
        formName: f.formName,
        act: f.act,
        description: f.description,
  ...
  const totalPendingForms = siteDigests.reduce((sum, s) => sum + s.forms.length, 0);

  if (!siteDigests.length) {
    return {
      skipped: true,
      reason: 'No pending forms found.',
      periodLabel,
      sitesConsidered: siteRows.length,
      emailsSent: 0,
      emailsFailed: 0,
  ...
  if (dryRun) {
    return {
      ...
      emailsSent: 1,
      emailsFailed: 0,
      sitesWithPending: siteDigests.length,
      totalPendingForms,
      recipientEmails,
      details: siteDigests
    };
  ...
  const html = buildConsolidatedPendingFormsHtml({ periodLabel, asOfLabel, siteDigests });
  const subject = `Pending forms notification · ${periodLabel} · ${siteDigests.length} site(s)`;
  ...
  try {
    ...
    console.log(
      `pendingFormsMailer: sent pending forms to configured recipients only (${recipientEmails.join(', ')}); ${totalPendingForms} form(s) across ${siteDigests.length} site(s)`
    );
    return {
      skipped: false,
      fromEmail,
    ...
    console.error('pendingFormsMailer: failed to send to configured recipients:', err?.message || err);
    return {
      skipped: false,
      fromEmail,
      periodLabel,
...
module.exports = {
  ...
  sendPendingFormsToRecipients,
  buildConsolidatedPendingFormsHtml,
  resolveEmailLogoUrl,
  vayonaLogoHtml,
