...
app.get('/sitemanagement', async (req, res) => {
  try {
    ...
    const rows = await zcql.executeZCQLQuery(`SELECT ROWID, SiteName, SiteAddress, SiteCity, SiteState, SitePostalCode, UNITNO, InchargeName, InchargePhone, InchargeEmail, InchargeDesignation, Industry, CREATEDTIME, MODIFIEDTIME FROM Site ORDER BY ROWID DESC ${limitClause}`);
